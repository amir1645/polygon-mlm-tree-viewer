...
function generateNodeHTML(node, level) {
    ...
    let html = `
        <div class="level level-${level}">
            <div class="member-node ${node.isCurrentUser ? 'current-user' : ''} ${hasChildren ? 'has-children' : ''} ${node.isExpanded ? 'expanded' : ''}" 
    ...
    if (node.isExpanded && hasChildren) {
        html += `<div class="children-container level-${level}">`;
        ...
        if (hasLeftChild) {
            ...
            html += `
                <div class="child-branch left">
                    ...
                    ${leftChild ? generateNodeHTML(leftChild, level + 1) : '<div class="loading-node">در حال بارگذاری...</div>'}
                </div>
            `;
        }
        ...
        if (hasRightChild) {
            ...
            html += `
                <div class="child-branch right">
                    ...
                    ${rightChild ? generateNodeHTML(rightChild, level + 1) : '<div class="loading-node">در حال بارگذاری...</div>'}
                </div>
            `;
        }
